...
import { CATEGORIES, categoryOf, type LeadStatus } from "@/lead-engine/lib/types";
import { ButtonLink, Card, EmptyState, cn } from "@/lead-engine/components/ui";
import { FadeIn, Stagger, StaggerItem } from "@/lead-engine/components/motion";
import { LeadRow } from "@/lead-engine/components/lead-row";
...
export default async function LeadsPage({ searchParams }: { searchParams: Promise<{ f?: string; cat?: string }> }) {
  ...
  const href = (nf: string, nc?: string) => `/leady/leads?f=${nf}${nc ? `&cat=${nc}` : ""}`;
  ...
  return (
    <div>
      <FadeIn className="flex items-end justify-between gap-4">
        ...
        <ButtonLink href="/leady/add" variant="primary" size="sm">
