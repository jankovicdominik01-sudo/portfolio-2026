...
export default async function LeadPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  if (user.role === "caller") {
    return (
      <div>
        <Link href="/" className="mb-6 inline-flex items-center gap-1.5 text-sm text-white/40 hover:text-white">
  ...
  return (
    <div>
      <Link href="/leads" className="mb-6 inline-flex items-center gap-1.5 text-sm text-white/40 hover:text-white">
    ...
    return (
      <Card className="flex flex-col gap-5 p-6 sm:flex-row sm:items-center sm:justify-between sm:p-7">
        ...
        <div className="flex flex-wrap gap-2">
          <ButtonLink href={`/leads/${lead.id}/call`} variant="secondary">
            Volám sám
          </ButtonLink>
          <AnalyzeButton leadId={lead.id} label="Znova analyzovať" variant="ghost" />
        </div>
...
function OfferFootnote({ has, category }: { has: boolean; category: string }) {
  ...
  return (
    <p className="mt-8 text-center text-[12px] text-white/25">
      ...
      <Link href="/settings" className="underline underline-offset-2 hover:text-white/60">
        Ponuky
      </Link>
    </p>
  );
