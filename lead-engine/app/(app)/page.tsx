...
import { requireUser, adminName } from "@/lib/auth";
  ...
  return (
    <FadeIn delay={delay}>
      <Card className="p-2">
        ...
        {leads.length > 5 ? (
          <Link href="/leads" className="block px-4 pb-3 text-sm text-white/40 hover:text-white">
      ...
      <div className="relative flex flex-col gap-6 sm:flex-row sm:items-end sm:justify-between">
        ...
        <div className="flex shrink-0 gap-2">
          {isCall && lead.company.phone ? (
            <ButtonLink href={`/leads/${lead.id}`} variant="ok" size="lg">
              <Phone className="size-4" /> Pripraviť hovor
            </ButtonLink>
          ) : (
            <ButtonLink href={`/leads/${lead.id}`} variant="primary" size="lg">
